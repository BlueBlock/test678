...
if ($UpdateChannel -ne "" -and $MainRepository -ne "" ) {
    ...
    if (Test-Path -Path "mRemoteNG\Properties\AssemblyInfo.cs") {
        ...
        Set-GitHubContent -OwnerName "BlueBlock" -RepositoryName "test678" -Path "test678\Properties\AssemblyInfo2.cs" -CommitMessage "AssemblyInfo.cs updated for  $UpdateChannel $ModifiedTagName" -Content $assemblyinfocs_content -BranchName main
